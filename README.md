...
InterviewGuide 是一个面向求职场景的智能面试辅助平台，覆盖简历解析、面试出题、知识库问答和评估报告生成。
系统结合 LLM、向量检索和流式交互能力，为求职者与 HR 提供更高效的面试准备与复盘体验。

## 核心亮点

- **简历分析自动化**：支持 PDF / DOCX / DOC / TXT 解析，结合内容哈希去重与异步任务处理。
- **模拟面试闭环**：基于简历内容生成主问题与 Follow-up 追问，支持连续问答与结果汇总。
- **RAG 知识库问答**：使用 PostgreSQL + pgvector 构建向量检索能力，支持文档分块、召回与流式问答。
- **稳定的 LLM 调用封装**：统一处理结构化输出重试、错误注入与流式输出归一化。
- **工程化交付**：后端、前端、对象存储、Redis、数据库均支持 Docker 一键部署。
...
| Spring AI 2.0 | LLM 集成 |
| PostgreSQL + pgvector | 关系存储 + 向量检索 |
| Redis（Redisson） | 缓存与异步消息 |
| Apache Tika | 文档解析 |
| MapStruct | 实体映射 |
| iText 8 | PDF 导出 |
